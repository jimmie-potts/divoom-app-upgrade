...
const repeats=3,samples=1000,children=new Set();
 ...
 for(const concurrency of [1,10,50]){
  ...
  for(let repeat=0;repeat<repeats;repeat++){
   ...
   try{
    ...
    receipt.passed&&=passed;receipt.profiles.push({concurrency,repeat,coldMs,p95Ms:p95,p99Ms:p99,maxMs:times.at(-1),hostHarnessPeakRssMiB:peakRss/1048576,admitted:state.snapshot.revision,overloadRejected:state.admissionRejected,passed});
...
}catch(error){receipt.passed=false;receipt.failure=['hook-timeout','hook-failed','unaccounted-admission'].includes(error.message)?error.message:'measurement-failed';}finally{await Promise.all([...children].map(child=>new Promise(resolve=>{child.once('exit',resolve);child.kill('SIGKILL');})));await rm(directory,{recursive:true,force:true});}
