...
export function Workspace(){
 ...
 return <div className="workspace"><div className="player-summary"><div><p><strong>{controller.sample?.value.session?.playlist.name??'Ready for a playlist'}</strong></p><p className="muted">{controller.sample?.value.player.state??'Loading player…'} · Simulator only</p></div><p>{controller.connected?'Live state connected':'Live state disconnected. Reconnecting…'}</p></div>
 {controller.error&&<p role="alert">{controller.error}</p>}{controller.pending&&!controller.busy&&<div className="notice"><p>Command outcome uncertain. Retry the original command or reconcile current state before choosing a new action.</p><div className="actions"><button disabled={controller.busy} onClick={controller.retry}>Retry command</button><button disabled={controller.busy} onClick={controller.reconcile}>Reconcile and discard</button></div></div>}
 <nav aria-label="Controller views">{['Library','Playlists','Player','Settings'].map(name=><button key={name} aria-current={tab===name?'page':undefined} onClick={()=>setTab(name)}>{name}</button>)}</nav>
 ...
 <div hidden={tab!=='Player'}><PlayerPanel active={tab==='Player'} controller={controller} selected={selected}/></div><div hidden={tab!=='Settings'}><Settings controller={controller}/></div></div>;
}
