...
interface Device {mode:'simulator'|'device';connected:boolean|null;availability:'unknown'|'available'|'offline';configuration:DeviceConfiguration|null;activeConfiguration:DeviceConfiguration|null;restartRequired:boolean;activeProfile:Rendition['profile'];profiles:Rendition['profile'][]}
export function Settings({controller}:{controller:Controller}){
 ...
 <div className="editor-layout"><form className="panel" onSubmit={e=>{e.preventDefault();void run(async()=>{const parsed=deviceConfiguration.safeParse({ip,profile,...(model.trim()?{model:model.trim()}:{}),...(firmware.trim()?{firmware:firmware.trim()}:{})});if(!parsed.success){setError('Enter a canonical private IPv4 address and valid profile/observations.');return;}const saved=await request<Device>('/device','PUT',parsed.data);setDevice(saved);setNotice(saved.mode==='simulator'?'Configuration saved. Simulator remains active.':saved.restartRequired?'Configuration saved. Restart the backend to apply it.':'Configuration saved. It matches the active settings.');});}}>
 ...
 <button disabled={disabled||!device} onClick={()=>void run(async()=>{await request('/device/probe','POST',{});await load();setNotice(physical?'Device probe completed. Transport responded; visible output is unverified.':'Simulator probe completed. Physical connectivity is unverified.');})}>{physical?'Probe device':'Probe simulator'}</button>
