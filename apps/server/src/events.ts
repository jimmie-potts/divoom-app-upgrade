...
interface Client {pool?:Set<Client>;reply:FastifyReply;blocked:boolean;timer?:ReturnType<typeof setTimeout>}
export class Events {
 ...
 private remove(client:Client):void {this.clients.delete(client);client.pool?.delete(client);if(client.timer)clearTimeout(client.timer);}
 ...
 register(app:FastifyInstance,path='/api/events'):void {
  ...
  app.get(path,(request,reply)=>{
   ...
   this.publish();
   const client:Client={reply,blocked:false,pool:shared};this.clients.add(client);shared.add(client);reply.hijack();
