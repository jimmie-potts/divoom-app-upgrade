...
export async function startServer(env: NodeJS.ProcessEnv = process.env) {
  ...
  const app = createApp({ webRoot, dataDir:config.dataDir, runtime:config,mcpEnabled:config.mcpEnabled??false,controllerEnabled:config.controllerEnabled??false,...(config.controllerIdentity?{controllerIdentity:config.controllerIdentity}:{}) });
  try {
