...
export type {PlaybackCheckpoint} from '@pixoo/library';
export interface PlaybackStore {
  claim():()=>void;
  capture(playlistId:string):Promise<PlaybackCheckpoint>;
...
export class PlaybackError extends Error {
  constructor(readonly code:'invalid-input'|'closed'|'busy'|'storage-error'|'no-context'|'screen-off'){super(code);this.name='PlaybackError';}
}
